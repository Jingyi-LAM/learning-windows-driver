...
)
{
	...
	case IOCTL_SAYHELLO:
		...
		{
			DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "[%s] WdfRequestRetrieveInputBuffer Failed\n", MOD);
...
)
{
    ...
    if (!NT_SUCCESS(status))
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "[%s] WdfIoQueueCreate Failed\n", MOD);
